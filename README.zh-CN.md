...
在干净的 `master` checkout 中，用一条本地命令发下一版：

```bash
scripts/bump-version.sh 0.1.1
```

这个脚本会更新 `Cargo.toml` 和 `Cargo.lock`，提交版本 bump，并推送 `master`。
CI 通过后，`.github/workflows/auto-release.yml` 会在对应 tag 不存在时自动创建
`v0.1.1`，然后 dispatch `.github/workflows/release.yml`。Release workflow 会继续
发布 GitHub Release assets、GHCR 镜像、Docker Hub 镜像，以及 Homebrew/Linuxbrew
tap formula。

手工 tag 发版仍然支持，但 tag 必须和 `Cargo.toml` 匹配：
...
缓存 Cargo 和 Docker 层。
CI 的 Docker smoke build 和 release 的 Docker publish 使用独立写入 scope，但会互相读取
cache source；这样 master 上已经通过的 CI 可以预热 release 镜像构建，同时 release cache
又不完全依赖 CI。Release package job 也会缓存 Cargo registry/git 数据和 `target/`。
